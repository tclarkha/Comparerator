...
class EmployeeRoster:
    def __init__(self, master):
        ...
        # Stats Button
        self.info_button = Button(self.menu_frame, text="Stats", command=self.display_info, font=("Apple", 14), bg="gainsboro")
    ...
    def create_scrollable_text(self, master, label_text, row, column):
        ...
        text_widget = Text(frame, width=35, height=35, yscrollcommand=scrollbar.set, font=("Apple", 14))
    ...
    def count_entries(self, widget_key):
        label, text_widget = self.widgets[widget_key]
        text = text_widget.get(1.0, "end").strip()
        if not text:  # if text widget is empty
            return 0
        else:
    ...
    def display_info(self):
        try:
            ...
            total_joined = self.count_entries('joined')
            total_left = self.count_entries('left')
